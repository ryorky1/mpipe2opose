...
def generate_MP_JSON(cfg: DictConfig):
	...
	# paths
	skeleton_path = cfg.files.mpipe_json_output_path
	json_path = cfg.files.mpipe_skeleton_output_path
	# ---- Mediapipe config ----
	...
	# move images to new folder for later use
	for img in natsorted(img_folder):
		moved_img=cfg.files.moved_img_path + "/" + os.path.basename(img)
		moved_images.append(moved_img)
		os.rename(img, moved_img)
		log.info(f"image: {img}")
		log.info(f"original_image: {moved_img}")

	# Loop over all the sorted  media files in the folder
	for img in natsorted(moved_images):
		...
		with mp_pose.Pose(	min_detection_confidence	= 0.5,
			...
			if not landmarks_list:
				...
				# write keypoints_test_img to storage
				filename=img.replace(".","_keypoints.")
				keypoints_img=skeleton_path+"/"+filename.split("/")[-1]
				log.info(f"Filename2: {keypoints_img}")
				cv2.imwrite(keypoints_img, frame)
				...
				if cfg.params.write_json == True:
					...
					# keypoints_img to create json should be the original image not the keypoints_img ???
					keypoints_img=json_path+"/"+filename.split("/")[-1]
					json_filename = str(keypoints_img) + ".json"
